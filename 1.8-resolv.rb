    ...
    def each_resource(name, typeclass, &proc)
      ...
      begin
        @config.resolv(name) {|candidate, tout, nameserver|
          ...
          # TODO - why create the msg, if you might not need it?
    ...
    def extract_resources(msg, name, typeclass)
      ...
      msg.each_answer {|n, ttl, data|
        if n0 == n
          case data
          when typeclass
            # yield this (matching) RR for name
            yield data
            yielded = true
          when Resource::CNAME
            # typeclass doesn't match, but this name is aliased to another...
            n0 = data.name
          end
        end
      }
      return if yielded
      # The CNAME alias could point to a RR that we'd already passed by the time
      # we saw the CNAME, so go through again.
      msg.each_answer {|n, ttl, data|
        if n0 == n
          case data
          when typeclass
...
# - this shouldn't be fatal, the next resolver might be able to resolve this name!
    ...
    class Name
      ...
      # True if the name's labels are equal, where the match is case-insensitive.
      # Note that the "absolute?" attribute must also match, so "example.com"
    ...
    class Resource < Query
      ...
      class TXT < Resource
        ...
        # TODO - does this deal with multiple strings in a TXT RR?
        def self.decode_rdata(msg)
          data = msg.get_string
